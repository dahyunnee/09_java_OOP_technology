...
class PolyShape{
	...
	void draw() {		//자녀 클래스가 상속받아서 오버라이딩할 메소드를 정의 
...
class PolyLine extends PolyShape{
	...
	void draw() {		//부모클래스를 자신에 맞게 재정의하여 사용
...
public class OOPEx10 {
	...
	public static void main(String[] args) {
		...
		shapes[0].draw();		//자식클래스의 재정의 된 메소드가 호출된다.
		shapes[1].draw();		//자식클래스의 재정의 된 메소드가 호출된다.
		shapes[2].draw();		//자식클래스의 재정의 된 메소드가 호출된다.
