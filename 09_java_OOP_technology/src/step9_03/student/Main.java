	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (selectMenu == 1) {
				...
				if (!controller.getInsert().checkId(getId)) {		//아이디 존재하는 경우
					...
					StudentVO tempStd = controller.getStDAO().select(getId);
					tempStd.setName(changeName);
					tempStd.setNum(changeNum);
					
					
					controller.setStDAO(controller.getStDAO());
				...
				if (!controller.getInsert().checkId(getId)) {		//아이디 존재하는 경우
					...
					controller.deleteStd().deleteStudent(tempStd);;
